...
async def upload_file(request):
    ...
    if request_form.version == "v6":
        try:
            ...
            sentry_sdk.capture_exception(e)
            raise SanicException(
                message="Error while loading model (This may be caused by trying to convert either the latest release 4.0 that isn't supported yet, or by releases 2.0 or 3.0, in which case, try to convert using the 'YoloV6 (R2, R3)' option).",
                status_code=517,
            )
    else:
