...
class YoloV6R1Exporter(Exporter):
    ...
    def load_model(self):
        ...
        if hasattr(model.detect, "obj_preds"):
            ...
            raise ValueError(
                f"Error while loading model (This may be caused by trying to convert either the latest release 4.0 that isn't supported yet, or by releases 2.0 or 3.0, in which case, try to convert using the 'YoloV6 (R2, R3)' option)."
            )
        ...
        for i in range(len(onnx_model.graph.node)):
            for j in range(len(onnx_model.graph.node[i].input)):
                if onnx_model.graph.node[i].input[j] in change_inputs:
                    output_i = (
                        change_inputs.index(onnx_model.graph.node[i].input[j]) + 1
                    )
