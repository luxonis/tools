...
export const upload = createAsyncThunk(
  ...
  async (act, thunk) => {
    ...
    try {
      ...
      switch (status) {
        case 517:
          throw Error("Error while loading model (This may be caused by trying to convert a newer version of YoloV6 - release 2.0 or 3.0, if that is the case, try to convert using the `YoloV6 (latest)` option).");
        case 518:
          let errorData = JSON.parse(String.fromCharCode.apply(String, new Uint8Array(error.response.data)))
          throw Error(errorData['message']);
        case 519:
          throw Error("Error while loading model (This may be caused by trying to convert an older version of YoloV6 - release 1.0, if that is the case, try to convert using the `YoloV6 (legacy - R1)` option).");
        case 520:
